...
export function GridMotion({
  items = [],
  gradientColor = 'hsl(var(--portfolio-gray))',
  className
}: GridMotionProps) {
  ...
  const totalItems = 28
  const defaultItems = Array.from({ length: totalItems }, (_, index) => `Achievement ${index + 1}`)
    ...
    const updateMotion = () => {
      ...
      rowRefs.current.forEach((row, index) => {
        if (row) {
          ...
          row.style.transition = `transform ${0.8 + inertiaFactors[index % inertiaFactors.length]}s ease-out`
        }
      })
    }
              ...
              {[...Array(7)].map((_, itemIndex) => {
                ...
                return (
                  <div key={itemIndex} className="relative">
                    <div className="relative h-full w-full overflow-hidden rounded-lg bg-portfolio-gray/20 border border-portfolio-cyan/10 flex items-center justify-center text-portfolio-text text-xl backdrop-blur-sm">
